...
interface ChatIntegrationExampleProps {
  ...
  sendMessageAPI?: (conversationId: string, content: string, attachments?: File[]) => Promise<unknown>;
...
const ChatIntegrationExample: React.FC<ChatIntegrationExampleProps> = ({
  ...
  const chat = useChat({
    userId,
    onSendMessage: async (conversationId: string, content: string, attachments?: File[]) => {
      if (sendMessageAPI) {
        const response = await sendMessageAPI(conversationId, content, attachments);
  ...
  const totalUnreadCount = chat.conversations.reduce((total, conv) => total + conv.unreadCount, 0);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {chatMode === 'simple' && chat.currentConversation && (
        <SimpleChatInterface
          ...
          onSendMessage={async (content: string, attachments?: File[]) => {
            if (chat.currentConversationId) {
              return chat.sendMessage(chat.currentConversationId, content, attachments);
            }
      ...
      <div className="absolute bottom-20 right-0 bg-white rounded-lg shadow-lg p-2 border">
        <button
          onClick={() => setChatMode(chatMode === 'modern' ? 'simple' : 'modern')}
