...
const AdminUsersPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Search, filter, verify, and deactivate users
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setFilters((f) => ({
                    ...f,
                    isVerified: e.target.value as Filters["isVerified"],
                  }))
                }
                className="flex-1 border rounded px-3 py-2 text-sm bg-white"
              >
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-700">
            ...
            <strong>{verifyModal.user?.name}</strong> as a worker?
          </p>
          <div className="flex gap-2 justify-end">
            <Button
              variant="outline"
              onClick={() => setVerifyModal({ open: false })}
            >
              Cancel
            </Button>
            <Button onClick={() => verify(verifyModal.user!._id)}>
              Verify
            </Button>
          </div>
        </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-700">
            ...
            <strong>{deactivateModal.user?.name}</strong>?
          </p>
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setDeactivateModal((m) => ({ ...m, reason: e.target.value }))
              }
          ...
          <div className="flex gap-2 justify-end">
            <Button
              variant="outline"
              onClick={() => setDeactivateModal({ open: false })}
            >
              Cancel
            </Button>
            <Button
              variant="danger"
              onClick={() => deactivate(deactivateModal.user!._id)}
            >
              Deactivate
            </Button>
          </div>
      ...
      {/* Change Role Modal */}
      <Modal
        isOpen={editRole.open}
        onClose={() => setEditRole({ open: false })}
        title="Change User Role"
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-700">
            Select a new role for <strong>{editRole.user?.name}</strong>.
          </p>
          <select
            ...
            onChange={(e) =>
              setEditRole((m) => ({
                ...m,
                newRole: e.target.value as UserItem["role"],
              }))
            }
          ...
          <div className="flex gap-2 justify-end">
            <Button
              variant="outline"
              onClick={() => setEditRole({ open: false })}
            >
              Cancel
            </Button>
            <Button
              onClick={async () => {
                ...
                await adminAPI.updateUser(editRole.user._id, {
