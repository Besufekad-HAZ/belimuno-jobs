...
import { Camera, User2, Briefcase, GraduationCap, FileText, Sparkles, FileUp, FileIcon, Trash2, Link2, X, Eye } from 'lucide-react';
...
type WorkItem = { company?: string; title?: string; startDate?: string; endDate?: string; description?: string };

type ExtendedUser = BaseUser & {
  isVerified?: boolean;
  isActive?: boolean;
  profile?: (BaseUser['profile'] & { avatar?: string; bio?: string; dob?: string | Date; cv?: { name?: string; mimeType?: string; data?: string } });
  workerProfile?: {
    skills?: string[];
    experience?: string;
    hourlyRate?: number;
...
const AddSimple: React.FC<{ onAdd: (value: string) => void; placeholder?: string }> = ({ onAdd, placeholder }) => {
  const [value, setValue] = useState('');
  return (
    <div className="flex gap-2">
      <Input placeholder={placeholder || 'Type and add'} value={value} onChange={(e)=>setValue(e.target.value)} />
...
const AddSkill: React.FC<{ onAdd: (skill: string) => void }> = ({ onAdd }) => {
  ...
  return (
    <div className="flex gap-2">
      <Input placeholder="e.g., Floor polishing, Deep cleaning" value={skill} onChange={(e)=>setSkill(e.target.value)} />
      <Button onClick={()=>{ onAdd(skill); setSkill(''); }}>Add</Button>
    </div>
  );
};

const AddEducationForm: React.FC<{ onAdd: (item: Education) => void }> = ({ onAdd }) => {
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<Education>({});
  return (
    <div>
      {!open ? (
        <Button variant="outline" onClick={()=> setOpen(true)}>+ Add education</Button>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <Input placeholder="School/University" value={form.school || ''} onChange={(e)=> setForm({...form, school: e.target.value})} />
          ...
          <Input placeholder="Start (YYYY-MM)" value={form.startDate || ''} onChange={(e)=> setForm({...form, startDate: e.target.value})} />
          <Input placeholder="End (YYYY-MM)" value={form.endDate || ''} onChange={(e)=> setForm({...form, endDate: e.target.value})} />
          <Input placeholder="Description" value={form.description || ''} onChange={(e)=> setForm({...form, description: e.target.value})} />
          <div className="flex gap-2">
            <Button onClick={()=> { onAdd(form); setOpen(false); setForm({}); }}>Save</Button>
...
const AddWorkForm: React.FC<{ onAdd: (item: WorkItem) => void }> = ({ onAdd }) => {
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<WorkItem>({});
  return (
    <div>
      {!open ? (
        <Button variant="outline" onClick={()=> setOpen(true)}>+ Add work</Button>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
...
const ProfilePage = () => {
  const [user, setUser] = useState<ExtendedUser | null>(getStoredUser() as ExtendedUser | null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (serverUser) {
          setUser(serverUser);
          setCvPreview(ensureDataUrl(serverUser.profile?.cv?.data, serverUser.profile?.cv?.mimeType));
          try {
            const safeUser: ExtendedUser = { ...serverUser };
            // Avoid storing huge base64 blobs in cookies
            if (safeUser.profile?.cv?.data && safeUser.profile.cv.data.length > 500_000) {
              const { name, mimeType } = safeUser.profile.cv;
              safeUser.profile = { ...safeUser.profile, cv: { name, mimeType } };
            }
  ...
  const profileCompletion = (() => {
    if (role === 'worker') {
      const factors = [
        ...
        !!(user.workerProfile?.experience && user.workerProfile.experience.trim().length >= 20),
  ...
  const updateLocalUser = (partial: Partial<ExtendedUser>) => {
    ...
    try {
      ...
      const incomingCvData = typeof (partial as unknown) === 'object'
        && partial !== null
        && 'profile' in (partial as Record<string, unknown>)
        && (partial as { profile?: { cv?: { data?: unknown } } }).profile?.cv?.data;
      if (typeof incomingCvData === 'string') {
        const incomingMime = (partial as { profile?: { cv?: { mimeType?: string } } })?.profile?.cv?.mimeType;
  ...
  const saveProfile = async (payload: ProfileUpdatePayload) => {
    setSaving(true);
    try {
  const res = await authAPI.updateProfile(payload as unknown as Record<string, unknown>);
      const updated = (res.data?.user || res.data?.data?.user) as ExtendedUser | undefined;
  ...
  const addSkill = async (skill: string) => {
    ...
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, skills: [...list, v] } };
    updateLocalUser(nextUser);
    await saveProfile({ workerProfile: { skills: nextUser.workerProfile?.skills } });
  };
  const removeSkill = async (skill: string) => {
    const list = (user.workerProfile?.skills || []).filter(s => s !== skill);
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, skills: list } };
    updateLocalUser(nextUser);
    await saveProfile({ workerProfile: { skills: list } });
  };

  const setExperienceSummary = async (summary: string) => {
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, experience: summary } };
    updateLocalUser(nextUser);
  ...
  const addCertification = async (cert: string) => {
    ...
    const v = cert.trim();
    if (!v || list.includes(v)) return;
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, certifications: [...list, v] } };
    updateLocalUser(nextUser);
    await saveProfile({ workerProfile: { certifications: nextUser.workerProfile?.certifications } });
  };
  const removeCertification = async (cert: string) => {
    const list = (user.workerProfile?.certifications || []).filter(c => c !== cert);
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, certifications: list } };
    updateLocalUser(nextUser);
  ...
  const addLanguage = async (lang: string) => {
    ...
    const v = lang.trim();
    if (!v || list.includes(v)) return;
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, languages: [...list, v] } };
    updateLocalUser(nextUser);
    await saveProfile({ workerProfile: { languages: nextUser.workerProfile?.languages } });
  };
  const removeLanguage = async (lang: string) => {
    const list = (user.workerProfile?.languages || []).filter(l => l !== lang);
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, languages: list } };
    updateLocalUser(nextUser);
  ...
  const addPortfolioLink = async (url: string) => {
    ...
    const v = url.trim();
    if (!v) return;
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, portfolio: [...list, v] } };
    updateLocalUser(nextUser);
    await saveProfile({ workerProfile: { portfolio: nextUser.workerProfile?.portfolio } });
  };
  const removePortfolioLink = async (url: string) => {
    const list = (user.workerProfile?.portfolio || []).filter(u => u !== url);
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, portfolio: list } };
    updateLocalUser(nextUser);
  ...
  const addEducationItem = async (item: Education) => {
    ...
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, education: [...list, item] } };
    updateLocalUser(nextUser);
    await saveProfile({ workerProfile: { education: nextUser.workerProfile?.education } });
  };
  const removeEducationItem = async (index: number) => {
    const list = [...(user.workerProfile?.education || [])];
    list.splice(index, 1);
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, education: list } };
    updateLocalUser(nextUser);
  ...
  const addWorkItem = async (item: WorkItem) => {
    ...
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, workHistory: [...list, item] } };
    updateLocalUser(nextUser);
    await saveProfile({ workerProfile: { workHistory: nextUser.workerProfile?.workHistory } });
  };
  const removeWorkItem = async (index: number) => {
    const list = [...(user.workerProfile?.workHistory || [])];
    list.splice(index, 1);
    const nextUser: ExtendedUser = { ...user, workerProfile: { ...user.workerProfile, workHistory: list } };
    updateLocalUser(nextUser);
    await saveProfile({ workerProfile: { workHistory: list } });
  };

  const onDOBBlur = async (value: string) => {
    await saveProfile({ profile: { ...user.profile, dob: value && value.trim() ? value : null } });
  };
          ...
          <Card className="flex-1">
            <div className="flex items-start gap-4">
              <div className="relative">
                ...
                <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e)=> e.target.files && onAvatarChange(e.target.files[0])}/>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Add a short, compelling summary about you.</p>
                </div>
                <div className="flex-1">
                  <textarea
                    rows={3}
                    defaultValue={user.profile?.bio || ''}
                    onBlur={(e)=> saveProfile({ profile: { ...user.profile, bio: e.target.value } })}
            ...
            {role === 'worker' && (
              <>
                <Card>
                  <h3 className="font-semibold text-gray-900 mb-2 flex items-center gap-2"><Briefcase className="h-4 w-4"/> Experience Summary</h3>
                  <p className="text-sm text-gray-600 mb-2">Summarize your relevant work experience, achievements, and tools.</p>
                  <textarea
                    rows={4}
                ...
                <Card>
                  <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2"><Sparkles className="h-4 w-4"/> Skills</h3>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(user.workerProfile?.skills || []).map((s, i) => (
                      ...
                      >
                        <Badge className="!bg-transparent !border-0 !px-0 !py-0" variant="secondary" size="sm">{s}</Badge>
                        <button
                          aria-label={`Remove ${s}`}
                ...
                <Card>
                  <h3 className="font-semibold text-gray-900 mb-2 flex items-center gap-2"><GraduationCap className="h-4 w-4"/> Certifications & Languages</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <h4 className="font-medium text-gray-900 mb-2">Certifications</h4>
                      <AddSimple onAdd={addCertification} placeholder="e.g., OSHA Safety, Cleaning Pro Level 2" />
                      <div className="mt-3 flex flex-wrap gap-2">
                        {(user.workerProfile?.certifications || []).map((c, i) => (
                          <span
                            key={i}
                            className="group inline-flex items-center gap-1 border border-gray-200 rounded-full bg-white px-2 py-1 shadow-sm"
                          >
                            <Badge className="!bg-transparent !border-0 !px-0 !py-0" variant="secondary" size="sm">{c}</Badge>
                            <button className="p-0.5 rounded-full text-gray-400 hover:text-red-600 hover:bg-red-50" onClick={()=> removeCertification(c)}>
                              <X className="h-3.5 w-3.5" />
                            </button>
                          </span>
                        ))}
                    ...
                    <div>
                      ...
                      <AddSimple onAdd={addLanguage} placeholder="e.g., Amharic, English" />
                      <div className="mt-3 flex flex-wrap gap-2">
                        {(user.workerProfile?.languages || []).map((l, i) => (
                          <span
                            ...
                            className="group inline-flex items-center gap-1 border border-gray-200 rounded-full bg-white px-2 py-1 shadow-sm"
                          >
                            <Badge className="!bg-transparent !border-0 !px-0 !py-0" variant="secondary" size="sm">{l}</Badge>
                            <button className="p-0.5 rounded-full text-gray-400 hover:text-red-600 hover:bg-red-50" onClick={()=> removeLanguage(l)}>
                              <X className="h-3.5 w-3.5" />
                            </button>
                          </span>
                        ))}
                ...
                <Card>
                  <h3 className="font-semibold text-gray-900 mb-2 flex items-center gap-2"><Link2 className="h-4 w-4"/> Portfolio</h3>
                  <AddSimple onAdd={addPortfolioLink} placeholder="https://your-portfolio-link" />
                  <ul className="mt-3 space-y-1 text-sm">
                    {(user.workerProfile?.portfolio || []).map((url, i) => (
                      <li key={i} className="flex items-center justify-between gap-3 p-2 bg-white rounded border border-gray-200 shadow-sm">
                        <a className="underline text-blue-700 truncate" href={url} target="_blank" rel="noreferrer">{url}</a>
                        <button aria-label={`Remove ${url}`} className="p-1 rounded-full text-gray-400 hover:text-red-600 hover:bg-red-50" onClick={()=> removePortfolioLink(url)}>
                          <X className="h-4 w-4" />
                        </button>
                      </li>
                    ))}
                ...
                <Card>
                  <h3 className="font-semibold text-gray-900 mb-2">Educational Background</h3>
                  <AddEducationForm onAdd={addEducationItem} />
                  <div className="mt-3 space-y-3">
                    {(user.workerProfile?.education || []).map((ed, i) => (
                      <div key={i} className="relative p-3 bg-white rounded border border-gray-200 shadow-sm">
                        <button
                        ...
                        <div className="font-medium text-gray-900">{ed.school} {ed.degree ? `• ${ed.degree}` : ''}</div>
                        <div className="text-sm text-gray-600">{ed.startDate || '—'} - {ed.endDate || 'Present'}</div>
                        {ed.field && <div className="text-sm text-gray-600">Field: {ed.field}</div>}
                  ...
                  <div className="mt-3 space-y-3">
                    {(user.workerProfile?.workHistory || []).map((wk, i) => (
                      <div key={i} className="relative p-3 bg-white rounded border border-gray-200 shadow-sm">
                        ...
                        <div className="font-medium text-gray-900">{wk.title} {wk.company ? `• ${wk.company}` : ''}</div>
                        <div className="text-sm text-gray-600">{wk.startDate || '—'} - {wk.endDate || 'Present'}</div>
                        {wk.description && <p className="text-sm text-gray-700 mt-1">{wk.description}</p>}
                      </div>
                    ))}
                  </div>
                </Card>
            ...
            {role === 'client' && (
              <>
                <Card>
                  <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2"><FileText className="h-4 w-4"/> Company</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <Input label="Company Name" defaultValue={user.clientProfile?.companyName || ''} onBlur={(e)=> saveProfile({ clientProfile: { ...user.clientProfile, companyName: e.target.value } })} />
                    <Input label="Website" defaultValue={user.clientProfile?.website || ''} onBlur={(e)=> saveProfile({ clientProfile: { ...user.clientProfile, website: e.target.value } })} />
                    <Input label="Industry" defaultValue={user.clientProfile?.industry || ''} onBlur={(e)=> saveProfile({ clientProfile: { ...user.clientProfile, industry: e.target.value } })} />
                  </div>
                </Card>
                <Card>
                  <h3 className="font-semibold text-gray-900 mb-2">Preferences</h3>
                  <p className="text-sm text-gray-600">Update notification and language preferences in settings.</p>
                </Card>
              </>
            )}
          ...
          <div className="space-y-6">
            <Card>
              ...
              <p className="text-xs text-gray-500 mt-2">Complete your profile to increase trust and win more jobs.</p>
              {saving && <p className="text-xs text-gray-500 mt-1">Saving...</p>}
            </Card>

            <Card>
              <h3 className="font-semibold text-gray-900 mb-2">Date of Birth</h3>
              <Input type="date" defaultValue={user.profile?.dob ? String(user.profile?.dob).substring(0,10) : ''} onBlur={(e)=> onDOBBlur(e.target.value)} />
            </Card>

    <Card>
              <h3 className="font-semibold text-gray-900 mb-2 flex items-center gap-2"><FileIcon className="h-4 w-4"/> CV</h3>
        {(() => { const cv = user.profile?.cv; return cv ? (
                <div>
            ...
            return (
              ...
              >
                ...
                <div className="text-sm text-gray-600">
                  ...
                  <Button onClick={()=> cvRef.current?.click()}><FileUp className="h-4 w-4 mr-2"/>Upload CV</Button>
                </div>
      ); })()}
              <input ref={cvRef} type="file" accept="application/pdf,.doc,.docx" className="hidden" onChange={(e)=> e.target.files && onCVFile(e.target.files[0])}/>
            </Card>
          </div>
        </div>
      </div>
