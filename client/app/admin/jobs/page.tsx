        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            ...
            <p className="text-gray-600">
              Search, review, and update job postings across the platform.
            </p>
          </div>
            ...
            <div className="relative">
              ...
              {query && (
                <button
                  aria-label="Clear"
                  onClick={() => setQuery("")}
                  className="absolute right-2 top-2.5 h-7 w-7 inline-flex items-center justify-center rounded hover:bg-gray-100"
                >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select
                  value={range}
                  onChange={(e) =>
                    setRange(e.target.value as "30d" | "90d" | "all")
                  }
            ...
            <table className="min-w-[1000px] w-full divide-y divide-gray-200">
              <thead className="bg-gray-50 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-[220px]">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-100">
                {filtered.map((job) => (
                  <tr key={job._id} className="hover:bg-gray-50/60">
                    <td className="px-6 py-4">
                      <div className="flex items-center min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-medium text-gray-900 truncate max-w-[420px]">
                            {job.title}
                          </div>
                          {job.deadline && (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                      {job.budget ? `ETB ${job.budget.toLocaleString()}` : "—"}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 text-xs rounded-full ${statusChips[job.status]}`}
                      >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex gap-2 justify-end">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            setShowDelete({ id: job._id, title: job.title })
                          }
                        >
        ...
        >
          <div className="space-y-4">
            <p className="text-sm text-gray-700">
              ...
              <span className="font-semibold"> {showDelete?.title}</span>? This
...
const JobForm: React.FC<JobFormProps> = ({ initial, onCancel, onSave }) => {
  ...
  const [status, setStatus] = useState<AdminJob["status"]>(
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <select
          value={status}
          onChange={(e) => setStatus(e.target.value as AdminJob["status"])}
          className="w-full px-3 py-2 border rounded-md bg-white text-gray-900"
        >
          <option value="draft">Draft</option>
          <option value="posted">Posted</option>
